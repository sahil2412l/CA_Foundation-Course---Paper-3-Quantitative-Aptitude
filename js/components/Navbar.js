// Navbar Component Module with Send Email & About Modals
function Navbar({ user, activeTab }) {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-fade-in">
          <div className="w-full max-w-lg rounded-2xl border border-indigo-500/30 bg-slate-900 p-6 shadow-2xl space-y-4 text-slate-100">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              ...
              <button onClick={() => setShowContactModal(false)} className="text-slate-400 hover:text-white text-xl font-bold p-1 cursor-pointer">✕</button>
            </div>
            ...
            {sentSuccess ? (
              ...
              <form onSubmit={handleSendEmailSubmit} className="space-y-3.5">
                ...
                <div className="flex justify-end gap-2.5 pt-2">
                  <button type="button" onClick={() => setShowContactModal(false)} className="rounded-xl border border-slate-700 bg-slate-800 px-4 py-2 text-xs font-semibold text-slate-300 hover:bg-slate-700 cursor-pointer">Cancel</button>
                  <button type="submit" className="rounded-xl bg-gradient-to-r from-indigo-600 to-indigo-500 hover:from-indigo-500 hover:to-indigo-400 px-5 py-2 text-xs font-bold text-white shadow-lg shadow-indigo-600/30 cursor-pointer transition">🚀 Send Email</button>
                </div>
              </form>
            )}
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-fade-in">
          <div className="w-full max-w-xl rounded-2xl border border-cyan-500/30 bg-slate-900 p-6 shadow-2xl space-y-4 text-slate-100 max-h-[85vh] overflow-y-auto">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              ...
              <button onClick={() => setShowAboutModal(false)} className="text-slate-400 hover:text-white text-xl font-bold p-1 cursor-pointer">✕</button>
            </div>
            <div className="space-y-3 text-xs leading-relaxed text-slate-300">
              <p>Welcome to the ultimate learning portal for <strong className="text-white">CA Foundation Paper 3 - Quantitative Aptitude</strong>. Designed to help aspirants score 80+ marks through structured chapter units, formula revision, and gamified practice.</p>
              ...
              <div className="rounded-xl border border-indigo-500/20 bg-indigo-950/30 p-3 text-slate-300 flex justify-between items-center">
                ...
                <button onClick={() => { setShowAboutModal(false); setShowContactModal(true); }} className="rounded-lg bg-indigo-600 px-3 py-1.5 text-xs font-bold text-white hover:bg-indigo-500 cursor-pointer">Contact Now</button>
              </div>
            </div>
            <div className="flex justify-end pt-2">
              <button onClick={() => setShowAboutModal(false)} className="rounded-xl bg-slate-800 hover:bg-slate-700 px-5 py-2 text-xs font-bold text-white cursor-pointer">Close</button>
            </div>
          </div>
        </div>
      )}
